...
def optimal_BST(p, q, n):  # p是关键字的概率(n个)， q是伪关键字的概率(n+1个)， n是关键字的个数
    ...
    for i in range(1, n + 2):  # 初始化e和w i属于[1,n+1]
        e[gk(i, i - 1)] = q[i - 1]
        w[gk(i, i - 1)] = q[i - 1]
    for l in range(1, n + 1):  # l代表的是字数的长度 l属于[1,n]
        for i in range(1, n - l + 2):  # i是长度为l的每个子树的关键字的最小的索引值 i属于[1,n-l+1]
            j = i + l - 1  # i是子树关键字的开始处，j是结束处的坐标，关键的索引是从1开始的，而p是从0开始的，需要做+1操作
            e[gk(i, j)] = MAX
            w[gk(i, j)] = w[gk(i, j - 1)] + p[j - 1] + q[j]
            for r in range(i, j + 1):  # r属于[i,j]
